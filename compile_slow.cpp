...
struct Game  {
	...
	bool is_legal ( int player ) {
		...
		for ( int i = 0; i < chk.size(); ++i ) if ( chk[i] == player ) return true;
		return false;
	}
...
// 	}
	...
	MoveRecommendation recommended_move (int player) {
...
int main() {
	...
	MoveRecommendation best_move = game.recommended_move(game.next_move);
	cout << 1095 << endl;
	game.apply_move(best_move.move.from, best_move.move.to);
	cout << 1096 << endl;
	game.process();
	cout << 1098 << endl;
	game.advance();
